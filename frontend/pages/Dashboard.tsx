...
const Dashboard = () => {
  const { stats, signals, isDark, toggleTheme, userType, setUserType, user, notifications, markAllNotificationsRead } = useApp();
      ...
      <header className="h-20 flex items-center justify-between px-8 sticky top-0 bg-background/80 backdrop-blur-md z-20 border-b border-transparent">
        ...
        <div className="flex items-center gap-4 ml-auto">
              ...
              {showNotifications && (
                <div className="absolute right-0 top-12 w-80 bg-white dark:bg-surface rounded-2xl shadow-m3-2 border border-surface-variant overflow-hidden z-50">
                  <div className="p-4 border-b border-surface-variant flex justify-between items-center">
                    ...
                    <button onClick={() => markAllNotificationsRead()} className="text-xs text-primary font-bold">Mark all read</button>
                  </div>
          ...
          <div className="relative">
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white transition-colors ${userType === 'Admin' ? 'bg-primary' : userType === 'Management' ? 'bg-purple-600' : 'bg-orange-600'
                }`}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
                {signals.map((signal) => (
                  <Link to="/case" key={signal.id} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-surface hover:shadow-sm border border-transparent hover:border-surface-variant transition-all cursor-pointer group">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-transform group-hover:scale-105 ${signal.riskLevel === 'Critical' ? 'bg-red-50 text-risk-high' :
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="mt-2 flex gap-1 justify-end">
                        <span className={`w-2 h-2 rounded-full ${signal.riskLevel === 'Critical' ? 'bg-risk-high' :
